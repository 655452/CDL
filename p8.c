...
#include<string.h>
#define TSIZE 128
// a tabela [i] [j] armazena o �ndice de produ��o que deve ser aplicado na i-�sima vari�vel se a entrada for j-�sima n�o-terminal
int table[100][TSIZE];
// armazena toda a lista de terminais do valor ASCII usado para indexar terminais terminais[i] = 1 significa que o caractere com valor ASCII � um terminal
char terminal[TSIZE];
// armazena todos os terminais
// apenas as letras mai�sculas de 'A' a 'Z' podem ser n�o terminais
// n�o terminais[i] significam que o i� alfabeto est� presente, j� que o n�o terminal � a gram�tica
char nonterminal[26];
//
struct product {
    char str[100];
...
// armazena o first de cada produ��o na forma A-> �
char first_rhs[100][TSIZE];
// verifica se o s�mbolo � um n�o terminal
int isNT(char c) {
    return c >= 'A' && c <= 'Z';
}
...
int main() {
    ...
    // mostra o first de cada vari�vel
    printf("\n");
    for (i = 0; i < no_pro; ++i) {
    ...
    // mostrar o follow de cada vari�vel
    printf("\n");
    for (i = 0; i < no_pro; ++i) {
        if (i == 0 || (pro[i - 1].str[0] != pro[i].str[0])) {
            char c = pro[i].str[0];
    ...
    // mostra o first de cada vari�vel � na forma de A->�
    printf("\n");
    for (i = 0; i < no_pro; ++i) {
